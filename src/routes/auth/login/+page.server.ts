...
export const load = async(event: PageServerLoadEvent) => {
    ...
    if ((token_id !== null && (typeof token_id === "string")) && 
    (exchange_code !== null && (typeof exchange_code === "string"))) {
        console.debug("\ttoken id = ", token_id);
        console.debug("\texchange code = ", exchange_code);
        console.debug("\t----requesting new access token-----");    
        
        const response = await fetch(`http://localhost:8082/oauth/exchange/${token_id}?exc=${exchange_code}`);
